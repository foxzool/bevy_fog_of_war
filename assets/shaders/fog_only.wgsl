// 迷雾效果着色器 - 独立版本，不需要外部输入纹理
// Fog effect shader - standalone version, no external input texture needed
...
fn fragment(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    let adjusted_fog_color = vec4<f32>(
        ...
        fog_intensity // 使用计算出的迷雾强度作为透明度 / Use calculated fog intensity as alpha
    );
    
    // 返回最终颜色
    // Return final color
    return adjusted_fog_color;
}
